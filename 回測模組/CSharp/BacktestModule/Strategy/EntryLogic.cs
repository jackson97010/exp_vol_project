...
namespace BacktestModule.Strategy
{
    ...
    public class EntryChecker
    {
            ...
            DateTime? lastExitTime = null)
        {
            ...
            {
                Console.WriteLine($"\n{"",60}");
                Console.WriteLine($"Entry Check | {stockId} | {currentTime:HH:mm:ss.fff}");
                Console.WriteLine($"Price: {currentPrice:F2} | DayHigh: {prevDayHigh:F2} | Ref: {refPrice:F2}");
            }
            ...
            bool aboveOpenEnabled = GetBool("above_open_check_enabled", true);
            ...
            {
                double massiveThreshold = GetDouble("massive_matching_amount", 1000000.0);
                // Dynamic threshold support (simplified: in full implementation, read from parquet)
                bool useDynamic = GetBool("use_dynamic_liquidity_threshold", false);
                if (!useDynamic)
                {
                    // Use fixed threshold
                }
                // else: dynamic threshold lookup would go here

                if (massiveMatchingAmount < massiveThreshold)
                {
                    signal.FailureReason = "Massive matching amount insufficient";
                    EntrySignals.Add(signal);
                    return signal;
                }
            }
            ...
            if (GetBool("interval_pct_filter_enabled", false))
            {
                int intervalMinutes = GetInt("interval_pct_minutes", 3);
                double intervalThreshold = GetDouble("interval_pct_threshold", 4.0);
            ...
            if (shouldLog)
            {
                Console.WriteLine("Entry check result: ALL CONDITIONS PASSED!");
            }
        ...
        public void PrintEntrySignalsSummary()
        {
            ...
            Console.WriteLine($"\n{"",60}");
            Console.WriteLine("Entry Signal Summary");
            Console.WriteLine($"Total checks: {totalChecks}");
            Console.WriteLine($"Passed: {passed}");
            Console.WriteLine($"Pass rate: {(double)passed / totalChecks * 100:F1}%");
            ...
            if (failureStats.Count > 0)
            {
                ...
                {
                    Console.WriteLine($"  {group.Key}: {group.Count()} ({(double)group.Count() / totalChecks * 100:F1}%)");
                }
            }
        }

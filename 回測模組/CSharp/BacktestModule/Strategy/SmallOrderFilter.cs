...
namespace BacktestModule.Strategy
{
    ...
    public class SmallOrderFilter
    {
            ...
            List<TickData> data, DateTime currentTime)
        {
            ...
            var tradeData = data.Where(d => d.Type == "Trade").ToList();
            ...
            List<TickData> data = null)
        {
            ...
            // Get ask 5-level total from previous tick
            double askTotal = 0;
            var tradeData = data.Where(d => d.Type == "Trade").ToList();
            var prevTrades = tradeData.Where(t => t.Time < currentTime).ToList();
            if (prevTrades.Count > 0)
            {
                var prevRow = prevTrades.Last();

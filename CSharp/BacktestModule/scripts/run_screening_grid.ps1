...
# Data: screening_results.csv (263 dates, ~41k stock-days)

$ErrorActionPreference = "Continue"
Set-Location "$PSScriptRoot/.."
$csvPath = ".\screening_results.csv"
$configPath = "configs/Bo_v2_modeC_limit_up_entry.yaml"
$baseOutput = "D:\C#_backtest\grid_results\screening"
...
Write-Host "=========================================="
Write-Host "  Screening Results Grid Backtest"
Write-Host "=========================================="
...
Write-Host "Base output: $baseOutput"
Write-Host "=========================================="
...
foreach ($ticks in $ticksList) {
    foreach ($window in $windowList) {
        $comboIndex++
        $outputPath = "$baseOutput\sl${ticks}_mm${window}s"
        Write-Host ""
        Write-Host "=========================================="
...
foreach ($ticks in $ticksList) {
    foreach ($window in $windowList) {
        $dir = "$baseOutput\sl${ticks}_mm${window}s"
        if (Test-Path $dir) {

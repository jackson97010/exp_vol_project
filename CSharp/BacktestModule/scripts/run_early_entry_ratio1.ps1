# run_early_entry_ratio1.ps1
# Early entry backtest: massive matching OFF, ratio > 1
#   2 entry times (0905, 0903) x 2 surge thresholds (2.5%, 2.0%) x 3 exit modes = 12 combos
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File scripts\run_early_entry_ratio1.ps1
...
$baseOutput = "D:\C#_backtest\early_entry_ratio1"

# Base config files (one per exit mode)
$configs = @(
    @{ Name = "modeC";     File = "configs/Bo_v2_early_ratio1_modeC.yaml" },
    @{ Name = "modeD_135"; File = "configs/Bo_v2_early_ratio1_modeD_135.yaml" },
    @{ Name = "modeD_357"; File = "configs/Bo_v2_early_ratio1_modeD_357.yaml" }
)
...
Write-Host "  Early Entry Ratio1 Backtest" -ForegroundColor Cyan
Write-Host "  massive_matching: OFF, ratio > 1" -ForegroundColor Cyan
...
try {
    foreach ($variant in $variants) {
        foreach ($cfg in $configs) {
            ...
            Write-Host "  Entry: $($variant.StartTime)~09:30, ratio>1, no massive matching" -ForegroundColor Yellow
            Write-Host "  Surge filter: 3min < $($variant.SurgeThreshold)%" -ForegroundColor Yellow
            if ($variant.RatioSkip) {
                Write-Host "  Ratio skip before: $($variant.RatioSkip)" -ForegroundColor Yellow
            }

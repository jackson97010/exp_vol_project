# B2_massive_no_filter_lu x 4 entry start times (0905/0906/0907/0908)
...
$baseOutput = "D:\C#_backtest\ModeE_full_compare"
$config = "configs/B2_massive_no_filter_lu.yaml"
...
foreach ($et in $entryTimes) {
    $outputPath = "$baseOutput\B2_massive_no_filter_lu_$($et.Suffix)"
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "Running: B2_massive_no_filter_lu entry=$($et.Time)"
    Write-Host "Config:  $config"
    Write-Host "Output:  $outputPath"
    Write-Host "Dates:   $($dates.Count)"
    Write-Host "============================================================"

    $cmd = "dotnet run --configuration Release -- --mode batch --dates $dateArgs --config `"$config`" --screening_csv `"$screeningCsv`" --tick_data `"$tickData`" --output_path `"$outputPath`" --entry_start_time $($et.Time)"
    Invoke-Expression $cmd
}

Write-Host ""
Write-Host "============================================================"
Write-Host "All 4 entry time variants completed!"
Write-Host "Results:"
foreach ($et in $entryTimes) {
    Write-Host "  B2_$($et.Suffix): $baseOutput\B2_massive_no_filter_lu_$($et.Suffix)"
}
Write-Host "============================================================"

# B組: bypass_group x ask>bid x 4 entry_times
# Only day high breakout + ask>bid + price_change_limit 8.5%, NO quality filters (massive/急拉/ratio)
# 4 entry times = 4 groups
...
$baseOutput = "D:\C#_backtest\ModeE_bypass_grid_B"
...
$totalGroups = $entryTimes.Count
$currentGroup = 0

foreach ($ei in 0..($entryTimes.Count - 1)) {
    $currentGroup++
    $label = "bypass_askgtbid_$($entryLabels[$ei])"
    $outputPath = "$baseOutput\$label"

    # Day high breakout + ask>bid + price limit 8.5%, no massive/急拉/ratio filters
    $overrideYaml = @"

# B組 overrides: no massive matching / 急拉 / ratio filters, keep ask>bid + price limit
  bypass_group_screening: true
  entry_start_time: "$($entryTimes[$ei])"
  massive_matching_enabled: false
  interval_pct_filter_enabled: false
  ratio_entry_enabled: false
  price_change_limit_enabled: true
  price_change_limit_pct: 8.5
  require_ask_gt_bid: true
"@
    ...
    Write-Host "  entry=$($entryTimes[$ei]) filters=ask>bid+price_limit_8.5%"
...
Write-Host ""
Write-Host "============================================================"
Write-Host "All $totalGroups B-group combinations completed!"
